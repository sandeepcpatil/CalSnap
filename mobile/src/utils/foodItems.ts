...
export function rescaleItem(item: FoodItem, quantity: number, unit: string): FoodItem {
  ...
  return {
    ...
    sat_fat_g: r1(item.sat_fat_g),
    sodium_mg: Math.round(num(item.sodium_mg) * k),
  };
}
...
export function sumItems(items: readonly FoodItem[]): ItemTotals {
  ...
  return {
    ...
    sugar_g: r1(items.reduce((s, i) => s + num(i.sugar_g), 0)),
    sat_fat_g: r1(items.reduce((s, i) => s + num(i.sat_fat_g), 0)),
    sodium_mg: Math.round(items.reduce((s, i) => s + num(i.sodium_mg), 0)),
...
interface FoodDef {
  ...
  f: number;
  fib: number;
  /** per 100 g — optional; only set where it materially matters (e.g. salt). */
  sug?: number;
  sat?: number;
  na?: number; // sodium in mg
}
...
export const COMMON_FOODS: readonly FoodDef[] = [
  ...
  { name: 'Ghee',                unit: 'tsp',    kcal: 900, p: 0,   c: 0,  f: 100, fib: 0, sat: 60 },
  { name: 'Cooking oil',         unit: 'tsp',    kcal: 884, p: 0,   c: 0,  f: 100, fib: 0 },
  { name: 'Paneer',              unit: 'katori', kcal: 265, p: 18,  c: 1.2, f: 21, fib: 0, sat: 14, na: 22 },
  ...
  { name: 'Samosa',              unit: 'piece',  kcal: 300, p: 5,   c: 37, f: 15, fib: 3, na: 420, sat: 6 },
  { name: 'Salad (raw veg)',     unit: 'katori', kcal: 25,  p: 1,   c: 5,  f: 0.2, fib: 2 },
  { name: 'Pickle / Achaar',     unit: 'tsp',    kcal: 180, p: 1,   c: 10, f: 15, fib: 2, na: 2500 },
  { name: 'Papad (roasted)',     unit: 'piece',  kcal: 350, p: 20,  c: 55, f: 3,  fib: 10, na: 1600 },
  ...
  { name: 'Bread (white)',       unit: 'slice',  kcal: 265, p: 9,   c: 49, f: 3.2, fib: 2.7, na: 490, sug: 5 },
  { name: 'Sugar',               unit: 'tsp',    kcal: 387, p: 0,   c: 100, f: 0, fib: 0, sug: 100 },
  { name: 'Sweet / Mithai',      unit: 'piece',  kcal: 350, p: 5,   c: 50, f: 14, fib: 1, sug: 40, sat: 7 },
];
...
export function itemFromFood(food: FoodDef, quantity: number, unit: string): FoodItem {
  ...
  return {
    ...
    fat_g: r1(food.f),
    fiber_g: r1(food.fib),
    sugar_g: r1(food.sug ?? 0),
    sat_fat_g: r1(food.sat ?? 0),
    sodium_mg: Math.round((food.na ?? 0) * k),
  };
}
